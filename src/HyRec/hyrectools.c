...
double ***create_3D_array(unsigned n1, unsigned n2, unsigned n3)
{
  ...
  for (i = 0; i < n1; i++)
    matrix[i] = create_2D_array(n2, n3);
...
double rec_interp1d(double x0, double dx, double *ytab, unsigned int Nx, double x)
{
  ...
  /* Return value */
  return (
      -ytab[0] * frac * (1. - frac) * (2. - frac) / 6. + ytab[1] * (1. + frac) * (1. - frac) * (2. - frac) / 2. + ytab[2] * (1. + frac) * frac * (2. - frac) / 2. - ytab[3] * (1. + frac) * frac * (1. - frac) / 6.);
}
                    ...
                    unsigned int Nx1, unsigned int Nx2, double x1, double x2)
{
  ...
  for (j = 0; j < 4; j++)
    temp[j] = rec_interp1d(x20, dx2, ytab[j], Nx2, x2);

  return (
      -temp[0] * frac1 * (1. - frac1) * (2. - frac1) / 6. + temp[1] * (1. + frac1) * (1. - frac1) * (2. - frac1) / 2. + temp[2] * (1. + frac1) * frac1 * (2. - frac1) / 2. + temp[3] * (1. + frac1) * frac1 * (frac1 - 1.) / 6.);
}
