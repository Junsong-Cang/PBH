...
void rec_get_cosmoparam(FILE *fin, FILE *fout, REC_COSMOPARAMS *param)
{
    ...
    */
    ...
    param->Gamma = 0.0;
    param->PBH_Model = 1;
    param->PBH_Distribution = 1;
    param->Mbh = 1e3;
    param->fbh = 0.0;
    param->PBH_Lognormal_Sigma = 1.0;
    param->PBH_PWL_Mmax = 1.0e16;
    param->PBH_PWL_Gamma = 0.5;
    param->PBH_Spin = 0.0;
    param->T0 = 2.728;
    param->obh2 = 0.02242;
    param->omh2 = 0.1424;
    param->okh2 = 0.0;
    param->odeh2 = 0.31538756;
    param->w0 = -1;
    param->wa = 0.0;
    param->Y = 0.245;
    param->Nnueff = 3.046;
                       ...
                       double *xe_output, double *Tm_output, double **Dfnu_hist, double *Dfminus_Ly_hist[3])
{
    ...
    double Delta_xe, xHeII, xH1s, d1, d3, d4;
    double **Dfminus_hist;
    int post_saha, idx_z;
    ...
    double dEdVdt_dep_vec_HIon[Redshift_Size], dEdVdt_dep_vec_LyA[Redshift_Size], dEdVdt_dep_vec_Heat[Redshift_Size];
    
    // Now fill dep arrays, scanf will be called nz*3 times
    for (idx_z = 0; idx_z < Redshift_Size; idx_z++)
    {
        scanf("%lf", &(dEdVdt_dep_vec_HIon[idx_z]));
    }
    for (idx_z = 0; idx_z < Redshift_Size; idx_z++)
    {
        scanf("%lf", &(dEdVdt_dep_vec_LyA[idx_z]));
    }
    for (idx_z = 0; idx_z < Redshift_Size; idx_z++)
    {
        scanf("%lf", &(dEdVdt_dep_vec_Heat[idx_z]));
    }
    ...
    */
    ...
    for (iz = 0; iz < param->nz && Delta_xe > 1e-8; iz++)
    {
        ...
        // Adding accreting PBH contribution
        d1 = Interp_1D(z, Redshift_Axis, dEdVdt_dep_vec_HIon, Redshift_Size, 2, 1);
        d3 = Interp_1D(z, Redshift_Axis, dEdVdt_dep_vec_LyA, Redshift_Size, 2, 1);
        d4 = Interp_1D(z, Redshift_Axis, dEdVdt_dep_vec_Heat, Redshift_Size, 2, 1);
        DarkArray[0] = DarkArray[0] + d1;
        DarkArray[1] = DarkArray[0] + d3;
        DarkArray[2] = DarkArray[0] + d4;
